...
本文将三角网格圈地博弈形式化为一个有限、确定、双人零和的图博弈。核心对象不是棋盘图像，而是由同色节点诱导的可见性图。边长为 $n$ 的三角网格只有
...
条潜在长程逻辑边。复杂度的主要来源正在这里：局面承载在二次规模的格点上，战术关系却发生在三次规模的可见性边集上。一次落子可能切断一条桥，进而删除不再连回基点的极大连通分量；价值变化因此由割点、桥和基点连通性决定，而不是由局部占点数线性决定。
...
$$
S_{\mathrm{aug}}(n)=2^{\Theta(5^{\Theta(n^2)})}.
$$
...
$$
d_{\max}(n)\le S_{\mathrm{pos}}(n)=2^{\Theta(n^2)}.
$$
...
=O\!\left((\Theta(n^2))^{2^{\Theta(n^2)}}\right).
$$
...
第二，语义状态空间与表示空间必须分开。$S_{\mathrm{pos}}(n)=2^{\Theta(n^2)}$ 是局面上界；$S_{\mathrm{repr}}(n)=2^{\Theta(n^3)}$ 是显式边缓存带来的表示上界。
